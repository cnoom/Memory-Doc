...
## 10. 背景名录（6 个，2026-09-02 全部发布）

`bg_map`(地图,记忆回廊：圆拱书架+烛光+记忆宝珠+中央留空,2026-09-02 补) / `bg_menu`(主菜单,图书馆大厅) / `bg_battle`(战斗,殿堂石厅烛光) / `bg_event`(事件,神秘壁龛) / `bg_shop`(商店,书房小店) / `bg_campfire`(篝火,休息壁龛)

> 每张 2 候选、vision 判读"中央 UI 留空区干净（stddev 采样）"者定稿；落选件在 `_archived/2026-09-01/backgrounds/`。命名与场景对应已随发布定稿（10 §9.2 `bg_` 前缀）。

## 10.5 装饰面板底图名录（panel_，新类别 2026-09-01）

`panel_map_sheet`(S04 地图页整版底图：磨损边羊皮纸+双线金边+四角叶饰金珠，中央留净；**已发布 2026-09-02**——目标≈0.6×源时直接整版 LANCZOS 缩放优于九宫格；磨损毛边版**仅整版等比缩放**——九宫格边带单轴拉伸会扭变形毛边，将来异形面板要九宫格需另出规则圆角边版本)
...
> `texture_parchment` 已发布（2026-09-02，S04 地图页材质薄叠在用）；AI 纹理偏淡，渲染与程序多倍频材质叠用。
...
`ui_mockup.py`（mh-art scripts）按 09 布局/色板把已发布资产合成 1920×1080 示意图，落 `ImageReview/mockups/` 校验后 `--publish` 发布（`mockup_` 前缀）。**2026-09-01 S01~S13 共 13 屏全量发布**（09 §5.1/§5.4/§6/§7.1~§7.4/§8.1~§8.3 嵌入）。字体为系统雅黑近似（正式实现 Noto Serif/Sans SC + TMP）；正面小卡文字按整卡缩放仅示意；商店屏遗物已换真图标（2026-09-02 v1.14，relic_mind_map/relic_charge_crystal）；卡牌奖励屏（S05）已换胜利牌匾+羊皮纸奖励页家族版（2026-09-05 09 v1.12，三卡 3:4 等比修正+卡下信息层）；Boss 遗物屏（S10）已换真图标收官（2026-09-05 09 v1.13/10 v1.16，Boss 三遗物 + 战利牌匾 + 羊皮纸页家族版）——**逐屏批次 S01~S13 十三屏全收官**。材质噪声（`_paper_material`/`paper_grain`）已去随机化——`Image.effect_noise` 吃进程内共享随机序列、同屏输出随已渲屏数漂移，已换 `_seeded_noise` 显式种子 numpy 噪声，全量渲=单渲逐字节可复现（2026-09-02；S05 发布时全量重渲 13 屏实测仅 s05 变化、其余 12 屏与正式版逐字节一致）。
