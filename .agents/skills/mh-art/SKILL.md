...
- **卡框（border_card_*，10§2.2/§4.3）**：`vertical card face frame design for a 3:4 card, flat and clean, the complete frame including its outer edge band kept fully inside the central safe area, leaving plain flat margins of about 10 percent of the canvas height at the top and bottom edges with absolutely no design elements there: warm cream parchment card face with a narrow <暗类型色> edge band, the illustration window shaped like a rounded arch window of a memory palace chapel, thin muted gold arch trim with one tiny gold star at the arch top, plain soft mint background inside the arch, lower area a clean empty cream panel, minimal, no characters, no text, no letters, no watermark`（v1.5 记忆殿堂拱窗版，2026-08-29；类型换色 attack=muted dark red / skill=muted dark navy blue / ability=muted deep violet / curse=muted dark charcoal grey 且窗内改 pale grey-lavender；整卡合成走 card_frame_mockup.py v4）
- **卡框管线错误复盘**：`references/frame-pipeline-pitfalls.md`——改 roundify_card.py / card_frame_mockup.py 或新增框体资产前先过一遍（14 条现象→根因→修复→防再犯）。
- **卡背/卡框发布口径（2026-08-29 定稿·分层合成）**：**类型色描边即卡框外缘，四类型描边等粗（26px）色准（锚定 10 §1.2 类型色板）**——`roundify_card.py --compose-type TYPE` 分层合成：程序描边环（RING_PALETTES 逐层圆角矩形，ability 含内金线层）+ 源图内容层（内缩 30px 取纯卡面、等比贴入零变形、奶油单色延伸补宽）。**拱形插画窗镂空（alpha=0）**：punch_arch_window 区域生长提取窗区（窗下缘锚定金框顶行），插画由引擎动态垫层。**禁止源图像素重绘描边**（勾线色/中位混合/手绘高光会色偏、产断续辅助线）。卡背走默认 60px 几何圆角居中裁切。生成 prompt 须带"中央 3:4 安全区+上下各留 ~10% 纯底空白"约束；**2:3 原图加工前先备份** `_archived/<日期>/source_2x3/`。错误全清单见 `references/frame-pipeline-pitfalls.md`（动管线前必读）。mockup 效果图产物自带 28px@480 圆角，布局按金线效果框实测 y≈465–617 排。
