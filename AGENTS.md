...
- 图片三目录流转：**预览** `ImageReview/<类型>/`（进 git，便于远端查看）→ **归档** `ImageReview/_archived/<日期>/<类型>/` → **实际应用** `docs/design/assets/`（进 git，扁平无子目录，文件名前缀区分类别，被设计文档嵌入）。详见 `ImageReview/README.md` 与 `docs/design/assets/README.md`。
