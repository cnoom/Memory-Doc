...
- 图片三目录流转：**预览** `ImageReview/<类型>/`（进 git，便于远端查看）→ **归档** `ImageReview/_archived/<日期>/<类型>/` → **实际应用** `Assets/UI/<类型>/`（项目根目录，进 git，**按类型分子目录**——与 ImageReview 类型目录同名，文件名前缀仍保留，被设计文档嵌入）。**发布必须先经用户确认拍板**：校验/vision 通过 ≠ 发布许可，示意图等一切生成资源先留在 ImageReview 待用户看过点头，才可 `--publish` 进 Assets/UI。详见 `ImageReview/README.md` 与 `Assets/UI/README.md`。
