...
本目录存放**校验通过并正式发布**的游戏美术资产，位于项目根目录 `Assets/UI/`（与将来 Unity 项目的资源路径一致），随仓库进 git，被 `docs/design/` 下的设计文档以相对路径嵌入（从任一设计文档引用均为 `../../Assets/UI/<文件>.png`，如 `![记忆水晶](../../Assets/UI/relic_memory_crystal.png)`）。
...
- 子目录：**无**——扁平目录，文件名前缀区分类别（`card_` / `cardback_` / `icon_` / `relic_` / `node_` / `enemy_` / `portrait_` / `bg_` / `texture_`）。
