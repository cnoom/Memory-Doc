...
| 路径 | 角色 | 规则 |
| --- | --- | --- |
| `<类型>/`（cards/ cardbacks/ cardframes/ icons/ relics/ potions/ map-nodes/ enemies/ portraits/ backgrounds/ textures/ logos/ mockups/ buttons/） | **预览**：当前待看的候选 | 只允许存在"当前待审批"与"明确挂起"的活候选 |
...
4. **用户拍板后**发布（移动到实际应用区对应类型子目录并按规范重命名，自动去掉 `_N` 候选后缀）。**校验/vision 通过 ≠ 发布许可**——必须等用户明确确认才能 `--publish`，示意图等一切生成资源同理：
   `python .agents/skills/mh-art/scripts/gen_asset.py --publish ImageReview/relics/relic_memory_crystal_1.png`
   （界面示意图用 `ui_mockup.py --publish`，落 `Assets/UI/mockups/`，同样须用户拍板。）
